...
> **STATUT : ITEMS 1 & 2 EXÉCUTÉS ET VÉRIFIÉS le 2026-07-11** (subagent-driven, double revue par tâche, deux revues finales : READY). **Item 3 (`repository create`, Tasks 9-12) NON EXÉCUTÉ — reste GATED en attente de l'arbitrage explicite de Jimmy.**
>
> **Suites** : vitest **162 passed / 23 fichiers**, 0 failed ; cargo `--lib` **110 passed**, 0 failed ; `npm run check` **0 errors, 0 warnings** (890 fichiers).
>
> **Captures réelles (faites par le contrôleur contre le CLI + serveur, avec cleanup)** :
> - **Item 2 / Task 1** `push_nonff.ndjson` : la course fabriquée (2e clone temp sur `feature/test` → commit+push distant → commit local → push refusé) donne un événement `error` `{"errorType":4294967295,"errorInner":"Branch has diverged, sync to merge remote changes"}` + `complete status 1`. **Variante 2A** — marqueur discriminant **`Branch has diverged`**. `push --fast-forward-merge` EXISTE mais le refus est une DIVERGENCE (le CLI dit « sync to merge ») → **variante 3B** (chaîne front sync→push). Constat critique : un sync de rattrapage propre auto-committe le merge et laisse `revisionMerged` **non-zéro en permanence** (2e parent) → `merge_in_progress` (= revisionMerged≠0, commands.rs:200) est trop large ; le garde de `syncAndPush` utilise **`stagedPending`** (revisionStaged≠0), seul signal fiable d'un merge non résolu (vérifié vs `status_merge.ndjson`). Repo de test restauré propre (rev 22 poussée), temp clone supprimé.
> - **Item 1 / Task 5** `shared_store_info.ndjson` + `_none.ndjson` : `shared-store info` émet `sharedStoreInfo{useAutomatically:0/1, remoteUrls[], paths[], exists[]}` (ARRAYS parallèles par remote ; ≠ hypothèse `path` singulier du plan), status 0 dans les deux cas. `create <remote-url>` REQUISE (erreur si store déjà existant → `enable` garde sur l'existence), `set-use-automatically <true|false>` GLOBAL, `clone` sans flag → **variante B** (toggle global AvatarMenu ; `enable` prend la server-url). Store créé sous `%LOCALAPPDATA%\Epic Games\lore\data\<remote>\shared_store` (laissé en place), `useAutomatically` remis à false.
>
> **Parcours navigateur mock validé** : (item 2) levier `pushNonFF` → Push → toast accent **« Remote has new changes »** + action **« Sync & push »** (PAS de toast rouge) ; clic → le sync vide le levier puis le push part (complétion throttlée par l'onglet caché — `visibilityState:hidden`, artefact d'environnement, pas un bug) ; (item 1) AvatarMenu → toggle **« Use shared store for clones »** décoché/actif par défaut → on (persiste à la réouverture du menu) → off (round-trip enable/disable). Console sans erreur.
>
> **Déviations conscientes vs draft du plan** : item 2 en variante 2A+3B avec garde `stagedPending` (le draft écrivait `mergeInProgress || stagedPending` — corrigé, `revisionMerged` permanent) ; item 1 en variante B avec champs ARRAYS et `enable(server_url)` (le draft supposait `path` singulier + `enable()` sans arg + éventuelle variante A) ; helpers `sharedStore.ts` (checkbox, variante A) NON créés. **Follow-up séparé consigné** (hors périmètre P6, pré-existant) : `merge_in_progress = revisionMerged != 0` allume le chip « Merge in progress » de façon fantôme après tout sync-merge committé — resserrer en `revisionMerged≠0 && revisionStaged≠0` (toutes les fixtures merge existantes ont les deux non-zéro, donc sans casse).
...
- **Cohérence de types** : `isNonFastForwardPush` défini Task 2, consommé Task 3 (26 occurrences cohérentes) ; `lore_shared_store_status/enable/disable` (Rust, Task 6) ↔ `sharedStoreStatus/Enable/Disable` (front, Task 7) ↔ `sharedStoreDefaultChecked`/`sharedStoreHint` (helpers, Tasks 7–8) ; `validateRepoName` défini Task 10, consommé Tasks 11–12 ; `lore_repository_create` Task 11 ↔ invoke Task 12.
